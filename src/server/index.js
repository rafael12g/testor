...
// --- Récupérer les balises validées d'une course ---

app.get('/api/ordre-balises/course/:courseId', async (req, res) => {
  const courseId = String(req.params.courseId || '').trim();
  if (!courseId) return res.status(400).json({ ok: false, error: 'courseId requis' });
  if (!isApiAvailable()) return res.status(503).json({ ok: false, error: 'API externe non disponible' });
  try {
    const authToken = getBearerToken(req);
    const balises = await getValidatedBalises(courseId, authToken);
    return res.json(balises);
  } catch (err) {
    return res.status(500).json({ ok: false, error: 'Erreur récupération balises' });
  }
});
