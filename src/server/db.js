...
// ─── Récupérer les balises validées d'une course depuis l'API externe ───

export async function getValidatedBalises(courseId, authToken = null) {
  if (!isApiAvailable()) return [];
  try {
    const balises = await apiFetch(`/api/ordre-balises/course/${courseId}`, {}, authToken);
    return Array.isArray(balises) ? balises : (balises?.data || balises?.value || []);
  } catch (err) {
    console.error(`[db] Erreur récupération balises validées pour course ${courseId}:`, err);
    return [];
  }
}