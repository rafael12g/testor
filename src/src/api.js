...
export async function fetchValidatedBalises(courseId) {
  try {
    const res = await safeFetch(`${BASE_URL}/ordre-balises/course/${courseId}`, {}, { fallback: [] });
    if (!res || !res.ok) return [];
    const data = await res.json();
    const balises = Array.isArray(data) ? data : (data.data || data.value || []);
    console.log('[api] fetchValidatedBalises result:', { courseId, balises, dataStructure: typeof data, isArray: Array.isArray(data) });
    return balises;
  } catch (err) { 
    console.error('[api] fetchValidatedBalises error:', err);
    return []; 
  }
}
