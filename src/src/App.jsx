...
import { fetchServerLogs, fetchRaceHistory, fetchCourses, fetchTeamByCode, setSessionToken, fetchValidatedBalises } from './api';
...
export default function OrienteeringApp() {
  ...
  // ── WebSocket pour les validations de balises ──
  const wsState = useWebSocket(user?.role === 'runner' && !!runnerSession?.raceId, async (msg) => {
    if (!runnerSession) return;
    console.log('[App] WebSocket message reçu:', msg, 'runnerSession:', runnerSession);
    if (msg.type === 'checkpoint_reached' && msg.payload) {
      console.log('[App] checkpoint_reached detected, checking if it matches our team...');
      // Vérifier si c'est pour notre course et notre équipe
      if (msg.payload.raceId === String(runnerSession.raceId) && msg.payload.teamCode === String(runnerSession.code)) {
        console.log('[App] Checkpoint for our team detected! Fetching validated balises...');
        // Récupérer les balises validées depuis l'API
        const validatedBalises = await fetchValidatedBalises(runnerSession.raceId);
        const countValidated = validatedBalises.filter(b => b.est_validee === true).length;
        console.log('[App] Validated balises count:', countValidated, 'current runnerProgress:', runnerProgress);
        if (countValidated !== runnerProgress) {
          console.log('[App] Updating runnerProgress from', runnerProgress, 'to', countValidated);
  ...
  // ── Polling régulier pour vérifier les balises validées (runner) ──
  useEffect(() => {
    if (user?.role !== 'runner' || !runnerSession?.raceId) return;
    
    const pollValidatedBalises = async () => {
      try {
        const validatedBalises = await fetchValidatedBalises(runnerSession.raceId);
        const countValidated = validatedBalises.filter(b => b.est_validee === true).length;
        console.log('[App] Polling: validated balises count:', countValidated, 'current runnerProgress:', runnerProgress);
        if (countValidated !== runnerProgress) {
          console.log('[App] Polling: Updating runnerProgress from', runnerProgress, 'to', countValidated);
          setRunnerProgress(countValidated);
        }
      } catch (err) {
    ...
    return () => clearInterval(interval);
  }, [runnerSession?.raceId, runnerProgress]);
